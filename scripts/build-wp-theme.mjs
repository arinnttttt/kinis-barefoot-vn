...
handleScroll();window.addEventListener('scroll',handleScroll,{passive:true});
  ...
  for (const route of routes) {
    ...
    bodyContent = bodyContent
      .replace(/\/videos\/kinis-showcase\.mp4/g, 'https://kinis.vn/wp-content/uploads/2026/03/pWF2AmF0GwAAAZ03ZYRcYWYAYXUOYXMaAJ4EgQ.mp4')
...
`;
    ...
    // Post-process front-page.php: inject dynamic testimonial + FAQ sections
    if (page.template === "front-page") {
      let fp = readFileSync(join(THEME_DIR, "front-page.php"), "utf8");

      // ---- Replace testimonial section with dynamic PHP ----
      const tStart = fp.indexOf('aria-labelledby="testimonial-heading"');
      if (tStart > -1) {
        const sectionStart = fp.lastIndexOf("<section", tStart);
        let depth = 0, i = sectionStart, sectionEnd = fp.length;
        while (i < fp.length) {
          if (fp.substr(i, 8) === "<section") { depth++; i += 8; }
          else if (fp.substr(i, 10) === "</section>") { depth--; if (depth === 0) { sectionEnd = i + 10; break; } i += 10; }
          else { i++; }
        }
        const origTestimonial = fp.substring(sectionStart, sectionEnd);

        const dynamicTestimonial = \`<?php
...
<?php endif; ?>\`;
        ...
        fp = fp.substring(0, sectionStart) + dynamicTestimonial + fp.substring(sectionEnd);
      }

      // ---- Replace FAQ section with dynamic PHP ----
      const fIdx = fp.indexOf('aria-labelledby="home-faq-heading"');
      if (fIdx > -1) {
        const fSectionStart = fp.lastIndexOf("<section", fIdx);
        let depth2 = 0, j = fSectionStart, fSectionEnd = fp.length;
        while (j < fp.length) {
          if (fp.substr(j, 8) === "<section") { depth2++; j += 8; }
          else if (fp.substr(j, 10) === "</section>") { depth2--; if (depth2 === 0) { fSectionEnd = j + 10; break; } j += 10; }
          else { j++; }
        }
        const origFaq = fp.substring(fSectionStart, fSectionEnd);

        const dynamicFaq = \`<?php
...
<?php endif; ?>\`;
        ...
        fp = fp.substring(0, fSectionStart) + dynamicFaq + fp.substring(fSectionEnd);
      }

      writeFileSync(join(THEME_DIR, "front-page.php"), fp);
    }
  }
