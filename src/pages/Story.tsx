  ...
  <Layout>
    ...
    <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8 bg-background">
      <div className="max-w-4xl mx-auto">
        <p className="text-base sm:text-lg text-muted-foreground leading-relaxed mb-10 sm:mb-16 animate-fade-in">
        ...
        <div className="space-y-8 sm:space-y-12">
          {milestones.map((m, i) => (
            <div
              key={m.year}
              className="flex gap-4 sm:gap-6 animate-fade-up"
              style={{ animationDelay: `${i * 100}ms` }}
            >
              <div className="flex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-secondary/10 flex items-center justify-center">
                <span className="font-display text-lg sm:text-xl font-bold text-secondary">{m.year}</span>
              </div>
              <div>
                <h3 className="font-display text-lg sm:text-xl font-semibold text-foreground">{m.title}</h3>
                <p className="mt-1.5 sm:mt-2 text-sm sm:text-base text-muted-foreground leading-relaxed">{m.desc}</p>
              </div>
            </div>
          ))}
        </div>
